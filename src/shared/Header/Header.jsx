    ...
    <nav className=" shadow">
      <div className="max-w-screen-2xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <p className='px-2 py-1 text-xs text-zinc-300 bg-zinc-800 rounded-full font-inter'>v1.0.1</p>
          <p className='px-2 py-1 text-xs text-zinc-100 font-semibold bg-zinc-800 rounded-full font-inter'>
            XenonUI v1.0.1 🔥</p>
        </div>
          ...
          <div className="flex space-x-6 items-center text-sm">
            {navLinks.map((link) => (
              <NavLink
                ...
                to={link.path}
                className={({ isActive }) =>
                  `  transition font-medium ${isActive ? 'text-[#DB3BF8]' : 'hover:text-zinc-300'
                  }`
                }
              >
                {link.name}
